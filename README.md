...
- **Truy vấn Công việc (tasks-query) (Implemented)**: Phân hệ `tasks-query` đã được tích hợp đầy đủ và kết nối trực tiếp với Firestore thật hoặc Fixture nội bộ dựa trên cấu hình môi trường (`TASKS_QUERY_SOURCE`). Đã chuẩn hóa toàn bộ cấu trúc thời gian của Firestore (Timestamp) sang định dạng chuẩn ISO String, ẩn triệt để các trường dữ liệu nhạy cảm phục vụ bảo mật đa vai trò (RBAC) chặt chẽ.
- **Ghi nhận Công việc (tasks-command) (Implemented - in-memory foundation)**: Đã kiến thiết thành công nền tảng ghi nhận dữ liệu (Command Path) cho phân hệ quản lý công việc. Hỗ trợ đầy đủ các giao thức nghiệp vụ: tạo mới (`POST`), chỉnh sửa (`PATCH`), phân công (`PUT /assignee`), chuyển trạng thái (`POST /transitions`) và lưu trữ công việc (`DELETE`) dựa trên cơ chế Optimistic Concurrency Control (OCC) thông qua `expectedVersion` và ghi nhật ký kiểm toán (Audit Trail) đồng bộ `requestId`. *Lưu ý*: Giao tiếp ghi dữ liệu thật xuống Firestore (Firestore write path) hiện tại được đánh dấu là NOT_IMPLEMENTED, dự kiến sẽ được triển khai vào giai đoạn G6.1+.
